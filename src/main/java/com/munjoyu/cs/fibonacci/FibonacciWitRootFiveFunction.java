...
public class FibonacciWitRootFiveFunction {

    private static final double GOLDEN_RATIO = (1 + Math.sqrt(5)) / 2.0d;
    private static final double GOLDEN_COUNTERPART_RATIO = (1 - Math.sqrt(5)) / 2.0d;
    ...
    private static long fibonacci(long n){
        if(n == 0 || n == 1) {
            ...
            return Math.round((Math.pow(GOLDEN_RATIO, n-1) - Math.pow(GOLDEN_COUNTERPART_RATIO, n-1)) / Math.sqrt(5));
        }
    }
